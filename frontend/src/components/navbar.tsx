...
const Navbar = () => {
  return (
    <nav className="w-20 bg-white shadow-lg flex flex-col items-center py-4 space-y-8">
      {/* Logo Section */}
      <div className="mb-4">
        <img
...
const NavButton = ({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) => (
  <Link href={href}>
    <button
      className="w-12 h-12 flex items-center justify-center text-gray-600 hover:text-blue-500 hover:bg-gray-100 rounded-lg transition-colors duration-200"
      title={label}
    >
      {icon}
    </button>
